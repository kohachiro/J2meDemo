...
class MainCanvas extends GameCanvas implements Runnable {
	...
	void drawMap(Graphics g) {
		...
		for(byte j=0;j<num_y*2-1;j++){
			...
			if(j==num_y-1){
				drawChar(g);
				//g.setColor(0xFFFFFF);
				//g.drawString(num_x+","+num_y,getWidth()/2-1,getHeight()/2-1-char_h/2-12,33);
			}
		}
	}
	...
	void charMove() {
		System.out.println(char_x+","+char_y);
		if(key==UP_PRESSED){
			if(towards!=0){
				towards=0;
			...
			sprite.nextFrame();
			if (char_y-1>=0&&map[char_x][char_y-1]>balk)
				char_y--;
			...
			sprite.nextFrame();
			if (char_x+1<map_w&&map[char_x+1][char_y]>balk)
				char_x++;
		}else if(key==DOWN_PRESSED){
			if(towards!=2){
				towards=2;
			...
			sprite.nextFrame();
			if (char_y+1<map_h&&map[char_x][char_y+1]>balk)
				char_y++;
			...
			sprite.nextFrame();
			if (char_x-1>=0&&map[char_x-1][char_y]>balk)
				char_x--;
		}else{
			sprite.setFrame(0);
		}
